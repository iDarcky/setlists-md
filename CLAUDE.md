...
- Chart display lives in one **`AaMenu.jsx`** popover (Lyrics/Chords/Page tabs) opened from the chart header; it writes the existing `settings` keys plus per-element colour (`chartLyricColor`/`chartChordColor`, fixed `CHART_COLOR_PALETTE`). Those colours **override the theme's text/chord in `useChartTheme`** (falsy = follow theme) and are portable prefs. The Layout `BottomSheet` remains as the "Advanced" backstop. `VIEW_MODES` lives in `ui/viewModes.js` (shared by `ViewModePicker` + the chart's ⋮ menu) so a constant export doesn't trip react-refresh. `StageHeader` has an `actionsInTitle` prop (Chart-only) that rides the actions on the title row. Not yet applied to Performance/Practice.
