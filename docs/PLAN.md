...
> | **Next** | **The two big threads, together** — graduate the `unifiedReader` flag (swap the call sites, keep `ChartView` in the tree) **and** collapse Practice/Live into one reader with a lock. They are ONE change: both rewrite the same seven branches in `App.jsx` (the three setlist routes + two finales), so doing them apart means writing those lines twice. ⚠ Flipping the flag's default does NOT graduate anyone — `loadSettings` merges defaults under stored settings and `saveSettings` persists the merged object, so every existing profile already has `unifiedReader: false` written down. Then **8 — key change**. |
...
> | **Then** | **8 → 13** — modulate, tabs, getting to the next song, chord diagrams, the practice tools, and 13. All BUILT and shipping; none has had its element pass. ⚠ Element 12 carries a known correction: **the metronome must not start on tap** (item 1 below). |
