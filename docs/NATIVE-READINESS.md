...
|---|---|---|---|
| N-1 | **PDF export uses `window.open('about:blank')` + `document.write`** | `src/pdf/exportSongPdf.js`, `src/pdf/exportSetlistPdf.js` | Already broken in iOS standalone PWA; in a Capacitor/TWA webview there is no popup at all. Needs the planned inline-iframe (or print-to-blob + native share) path **before** any store build. This is the single biggest shared blocker. |
