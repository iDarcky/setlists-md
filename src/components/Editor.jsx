...
export default function Editor({ song, onSave, onBack, onDirtyChange, importProgress, customSectionTypes, readOnly = false, chartDefaults = {}, initialArrangementId = null }) {
  ...
  // New songs open on Details to fill in metadata first; editing an existing
  // song opens on Arrange.
  const [activeTab, setActiveTab] = useState(() => (!song ? 'details' : 'arrange'));
  ...
  // The preview gets the real "Aa" display popover, wired to the
  ...
  // A FAITHFUL preview — reads the real global display settings and writes
  // through them (the Aa popover below edits global, same as the chart).
  ...
  // Song Hub's reader card. The Arrange/Tabs/Details tabs live as the left
  // card's pill header. No Advanced tab — raw markdown lives in the Source dialog.
  ...
  // Save/Cancel content — rendered in the bottom card.
  ...
  return (
    <div className="h-full bg-[var(--ds-background-200)] flex flex-col">
      ...
      {/* Source — raw-markdown editor. Reuses WriteTab so the toolbar's
          paste-import comes along. Edits the same body via setBody. */}
