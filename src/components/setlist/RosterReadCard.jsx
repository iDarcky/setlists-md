...
/**
 * Read-only "Who's playing" card for the setlist viewer. Everyone on the team
 * can see it; editing the roster stays in RosterPanel (admin-only). Renders
 * nothing when there's no team or no one is rostered for this setlist.
 */
export default function RosterReadCard({ setlistId, setlistDate }) {
  ...
  if (!team) return null;
  const dbSetlistId = setlistIdMap[setlistId] || setlistId;
  const rows = schedules
    ...
    .map(s => {
      ...
      return { ...s, member, status, name: member?.profile?.display_name || 'Member' };
    })
          ...
          return (
            <div key={r.id || r.user_id} className="flex items-center gap-3 px-4 py-2.5">
              <span className="relative shrink-0">
                <span className="grid place-items-center w-8 h-8 rounded-full bg-[var(--ds-gray-200)] text-label-12 font-semibold text-[var(--ds-gray-700)]">
