...
// A single toggle chip for a facet value (outlined pill).
...
function Section({ label, count = 0, defaultOpen, children }) {
  ...
  return (
    <div>
      ...
      >
        <span className="text-copy-14 font-medium text-[var(--text-1)] flex items-center gap-2">
          {label}
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && !isDesktop && createPortal(
        // Portaled to <body>, i.e. outside the page's modes subtree — re-declare
        // the variant here so --modes-* tokens resolve (otherwise borders fall
        // back to currentColor and render as stray white outlines).
