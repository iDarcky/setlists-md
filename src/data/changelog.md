...
- **One wrap-up screen at the end of a set (Labs)** — finishing a service or a practice now lands on a single screen instead of two near-identical ones. It shows **the set you just played**: the running order with the key each song was actually read in, and any key you moved marked on the song itself. Plus how long you ran, and — after a service in a Space — who you served with. Your reflection is saved for the **leaders only**: unlike before, it never reaches a band member's device at all. On your own, it stays with the setlist as it always did.
