...
- **Fix a chord without leaving the song (Labs)** — while you're editing in practice, tapping any chord opens the same chord picker the song editor uses, and the one you pick replaces the one you tapped. It's written back in the song's own key, so a chart you're reading in D still saves correctly.
...
- **The reader's top-bar icons sit together** as one group of tools instead of three loose buttons, which also gives the song title back some room.
