...
- **Editing a song holds still while you do it (Labs)** — the bottom navigation bar, the floating page arrows and the song counter all step out of the way instead of sitting on top of the editing bar, opening the editor closes the practice strip and greys out everything that would take you out of the song mid-change: the menu, the metronome, the setlist and the exit. Finish or discard first. Editing is marked with a solid orange bar across the top of the screen, which now reads the same on every chart theme instead of tinting the header a different amount on each one. Text no longer gets selected when you press and hold in the reader, so dragging works properly in the installed app.
...
- **The pinned section heading no longer hides behind the song map (Labs)** — turning the structure row on while a heading was pinned pushed it out of sight until you scrolled back up.
