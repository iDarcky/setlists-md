...
- **Fix a chord without leaving the song (Labs)** — while you're editing in practice, tapping any chord opens the song editor's chord bar: type any chord you like (slash chords included) or tap one of the suggestions from the song's key. On a phone it sits across the bottom of the screen instead of floating, and the suggestions wrap onto two short rows rather than scrolling sideways. The chord is written back in the song's own key, so a chart you're reading in D still saves correctly.
- **Editing a song holds still while you do it (Labs)** — opening the editor closes the practice strip and greys out everything that would take you out of the song mid-change: the menu, the metronome, the setlist, next/previous and the exit. Finish or discard first. Text no longer gets selected when you press and hold in the reader, so dragging works properly in the installed app.
- **Edit a section's words (Labs)** — every section gets a pencil beside its bin while you're editing: it opens the section's lyrics and chords as text, exactly as they're written, and saves them back. Tab blocks and key changes survive the round trip.
