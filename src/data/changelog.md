...
- **Editing a song holds still while you do it (Labs)** — the bottom navigation bar, the floating page arrows and the song counter all step out of the way instead of sitting on top of the editing bar, opening the editor closes the practice strip and greys out everything that would take you out of the song mid-change: the menu, the metronome, the setlist and the exit. Finish or discard first. Editing turns **the whole top of the screen** solid orange — the set across the top, the title bar and the song map together, rather than one orange band sitting between two normal ones. It's a fixed pair of colours, so it reads exactly the same on every chart theme instead of tinting a different amount on each one, and the song map's sections keep their own colours by filling in rather than fading out. Text no longer gets selected when you press and hold in the reader, so dragging works properly in the installed app.
