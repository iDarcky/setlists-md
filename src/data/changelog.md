...
- **One wrap-up screen at the end of a set (Labs)** — finishing a service or a practice now lands on a single screen instead of two near-identical ones, and it fits on one screen without scrolling. It shows how long you ran, who you served with after a service in a Space, and a place to write down what you want to remember. Two ways out — View setlist and Home — always visible. Your reflection is saved for the **leaders only**: unlike before, it never reaches a band member's device at all. On your own, it stays with the setlist as it always did.
