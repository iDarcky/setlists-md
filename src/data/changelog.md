...
- **On a phone the ☰ no longer covers the song (Labs)** — it used to slide up from the bottom over the chart. It now opens under the top bar and pushes the chart down, so nothing is hidden behind it and the song stays where you left it. Drag the handle at its bottom edge upwards to put it away.
