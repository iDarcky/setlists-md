...
- **The ☰ sheet drags on a phone (Labs)** — the little grab handle at the top was decoration; the only ways out were tapping outside it or pressing Escape. Now it follows your thumb: drag it up and it grows to fill most of the screen for the longer panels, drag it down and it steps back, drag it down again and it closes.
- **The ☰ rows are just the setting and an arrow (Labs)** — each row used to carry its current value on the right, which put four unrelated kinds of thing in one column, and one of them ("1 col") could never change on a phone.
