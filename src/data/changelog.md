...
- **The reader has its own menu now (Labs)** — the ☰ opens four rows instead of a display popover: **Display** (Look for themes, sizes, spacing, fonts and tabs; Layout for columns, structure, sections and how you move between songs), **The music** (who you're playing, chord names, sharps or flats, capo), **Notes** (the arrangement's note and every band cue in one place) and **The screen** (keep it awake). It's a sheet from the bottom on a phone and a panel beside the ☰ on a desktop, and the chart stays visible either way so you can see what you're changing. Practice stays where it was — one tap on the metronome, not buried in a menu.
