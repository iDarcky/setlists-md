...
- **The structure down the side no longer sits on the words (Labs)** — on the left or right it's now a column of soft dots that floats in the margin the chart already had: it doesn't cover a lyric and it doesn't push one aside either. Boxes and Chips are for the top and bottom, where there's room for them.
- **The dots had a dashed line inside them in Firefox (Labs)** — Firefox draws a tapped button's focus ring inside the shape, which on a 10px dot is a dashed circle through the middle of it. The ring now sits outside the dot, where it belongs.
