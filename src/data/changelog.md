...
- **One wrap-up screen at the end of a set (Labs)** — finishing a service or a practice now lands on a single screen instead of two near-identical ones: how long you ran, which keys you moved (and to what), and — after a service in a Space — who you served with. Your reflection is saved for the **leaders only**: unlike before, it never reaches a band member's device at all. On your own, it stays with the setlist as it always did.
